...
def test_frontier_v1_uses_double_log_radius_for_ratio():
    ...
    assert uncertainty_radius_multipliers(cfg) == {"eta80": 1.0, "eta120": 1.0, "ratio": 2.0}
    ...
    expected = (
        (abs(math.log10(3.4 / centres["eta80"])) + 0.05) ** 2
        + (abs(math.log10(0.4 / centres["eta120"])) + 0.05) ** 2
        + (abs(math.log10(8.2 / centres["ratio"])) + 0.10) ** 2
    )
    assert abs(score - expected) < 1e-14


def test_broad_interval_is_admissible_without_preferred_certification():
    cfg = _config()
    checks = robust_checks(canonicalize(_row(), cfg), cfg)
    assert bool(checks.iloc[0]["interval_inside_broad"])
    assert bool(checks.iloc[0]["domain_ratio_max"])

    strict = copy.deepcopy(cfg)
    strict["robustness"]["require_interval_inside_preferred"] = True
    strict_checks = robust_checks(canonicalize(_row(), strict), strict)
    assert not bool(strict_checks.iloc[0]["interval_inside_preferred"])


def test_legacy_configs_without_explicit_multipliers_keep_all_ones():
    cfg = _config()
    cfg.pop("uncertainty")
