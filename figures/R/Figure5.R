...
# A — property landscape on a true log scale so the decision-critical head is visible.
pA <- ggplot(tab, aes(mdi_fraction * 100, property_score)) +
  geom_point(size = 0.95, alpha = 0.20, colour = PUR_COL[["light"]]) +
  geom_point(data = tab %>% filter(feasible_nominal), size = 1.05, alpha = 0.42,
             colour = PUR_COL[["P"]]) +
  geom_vline(xintercept = 35, linetype = "22", linewidth = 0.55, colour = PUR_COL[["mid"]]) +
  geom_point(data = key, aes(colour = cid), size = 2.9, alpha = 1) +
  scale_colour_manual(
  ...
  labs(x = "MDI fraction of polyol + MDI (%)", y = "Nominal objective, J (log scale)") +
  annotate("text", x = 35.3, y = 0.55, label = "frozen 35% floor", hjust = 0,
...
phase <- bind_rows(
  ...
  mutate(
    ...
    width = xmax - xmin,
    short = sub("WO_INV_0", "", winner),
    label = case_when(
      winner %in% c("WO_INV_0419", "WO_INV_0579", "WO_INV_0420") ~ short,
      width >= 1.15 ~ short,
      TRUE ~ ""
    )
  )
...
pB <- ggplot(phase) +
  ...
  geom_text(data = phase %>% filter(label != ""), aes(x = (xmin + xmax) / 2, y = y, label = label),
            size = 2.00, colour = "white") +
  geom_vline(xintercept = 35, linetype = "22", linewidth = 0.60, colour = PUR_COL[["ink"]]) +
  annotate("text", x = 35.12, y = 2.47, label = "current floor", hjust = 0,
           size = 2.30, colour = PUR_COL[["ink"]]) +
...
# C — global ranks remain similar while the decision-critical head changes.
...
pC <- ggplot(ranked, aes(nominal_rank, robust_rank)) +
  ...
  annotate(
    ...
    hjust = 0, vjust = 1, size = 2.35, colour = PUR_COL[["ink"]]
  ) +
  annotate("text", x = 7, y = 10, label = "0579: 1 → 6", hjust = 0,
           size = 2.35, colour = PUR_COL[["P"]]) +
...
fig5 <- (pA | pB | pC) +
  plot_layout(widths = c(1.12, 1.08, 1.08)) +
  plot_annotation(tag_levels = "A")

save_pur_figure(fig5, "figures/final/Figure5_decision_frontier", width_mm = 180, height_mm = 86)
