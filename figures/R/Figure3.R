...
segments <- eff %>% transmute(family, xmin = pmin(multiplier_eta80, multiplier_eta120), xmax = pmax(multiplier_eta80, multiplier_eta120))
...
pA <- ggplot() +
  ...
  annotate("text", x = 0.665, y = 10.7, label = "Median 80 °C: 0.742", hjust = 0, size = 2.65, colour = PUR_COL[["T80"]]) +
  annotate("text", x = 0.665, y = 9.8, label = "Median 120 °C: 0.783", hjust = 0, size = 2.65, colour = PUR_COL[["T120"]]) +
  theme(legend.position = c(0.23, 0.08), legend.direction = "horizontal")

pB <- ggplot(eff, aes(slope_Ea_kJmol_per_pctNCO, family)) +
  geom_vline(xintercept = 0, linewidth = 0.45, colour = PUR_COL[["ink"]]) +
  geom_segment(aes(x = 0, xend = slope_Ea_kJmol_per_pctNCO, yend = family), linewidth = 0.8, colour = PUR_COL[["light"]]) +
  ...
  annotate("text", x = -3.05, y = 10.7, label = "9/11 negative", hjust = 0, size = 2.7, colour = PUR_COL[["ink"]])

pC <- ggplot(eff, aes(multiplier_eta120, multiplier_eta80, colour = polyol)) +
  geom_abline(slope = 1, intercept = 0, linetype = "22", linewidth = 0.45, colour = PUR_COL[["mid"]]) +
  geom_point(size = 2.25, alpha = 0.95) +
  scale_colour_manual(values = PUR_COL[c("P", "D", "C")]) +
  coord_equal(xlim = c(0.66, 0.97), ylim = c(0.66, 0.97)) +
  ...
  annotate("text", x = 0.925, y = 0.70, label = "Stronger low-T effect", hjust = 1, size = 2.55, colour = PUR_COL[["mid"]]) +
  theme(legend.position = c(0.19, 0.83), legend.direction = "horizontal")

fig3 <- pA + pB + pC + plot_layout(widths = c(1.25, 1.18, 0.95)) + plot_annotation(tag_levels = "A")

save_pur_figure(fig3, "figures/final/Figure3_free_NCO_coupling", width_mm = 180, height_mm = 78)
