...
pA <- ggplot(curves, aes(temperature_c, eta_norm, group = sample_id, colour = polyol)) +
  geom_line(alpha = 0.22, linewidth = 0.42) +
  geom_line(data = med_curves, aes(group = polyol), linewidth = 1.05, alpha = 0.98) +
  ...
  theme(legend.position = c(0.16, 0.17), legend.background = element_rect(fill = alpha("white", 0.85), colour = NA))

pB <- ggplot(fits, aes(r2, polyol, colour = polyol)) +
  geom_vline(xintercept = 0.98, linetype = "22", linewidth = 0.45, colour = PUR_COL[["mid"]]) +
  geom_jitter(width = 0, height = 0.12, size = 1.7, alpha = 0.9) +
  scale_colour_manual(values = PUR_COL[c("P", "D", "C")]) +
  scale_x_continuous(limits = c(0.84, 1.002), breaks = c(0.85, 0.90, 0.95, 0.98, 1.00), labels = label_number(accuracy = 0.01)) +
  labs(x = expression(Andrade~R^2), y = NULL) +
  annotate("text", x = 0.983, y = 3.35, label = "37/39 ≥ 0.98", hjust = 0, size = 2.7, colour = PUR_COL[["ink"]]) +
  theme(legend.position = "none")

pC <- ggplot(fits, aes(polyol, Ea_kJmol, colour = polyol)) +
  geom_boxplot(width = 0.50, outlier.shape = NA, linewidth = 0.50, fill = "white", colour = PUR_COL[["ink"]]) +
  geom_jitter(width = 0.11, size = 1.75, alpha = 0.88) +
  scale_colour_manual(values = PUR_COL[c("P", "D", "C")]) +
  scale_y_continuous(breaks = seq(30, 100, 10), expand = expansion(mult = c(0.03, 0.05))) +
  labs(x = "Polyol code", y = expression(E[a,app]~"(kJ mol"^{-1}*")")) +
  annotate("text", x = 1.02, y = 96.5, label = "Median = 51.09", hjust = 0, size = 2.7, colour = PUR_COL[["ink"]]) +
  theme(legend.position = "none")

fig2 <- pA + (pB / pC) +
  plot_layout(widths = c(1.62, 1.0), heights = c(1, 1)) +
  plot_annotation(tag_levels = "A")

save_pur_figure(fig2, "figures/final/Figure2_temperature_Andrade", width_mm = 180, height_mm = 98)
