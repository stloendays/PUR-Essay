...
# Keep the manuscript figure inside the ~40-80 °C source range.
tgrid <- seq(45, 75, by = 1)
curves <- fits %>%
  select(sample_id, polyol, A, B) %>%
  tidyr::crossing(temperature_c = tgrid) %>%
  mutate(
    eta_fit = exp(A + B / (temperature_c + 273.15)),
    eta_75 = exp(A + B / (75 + 273.15)),
    eta_norm = eta_fit / eta_75
  )
...
pA <- ggplot(curves, aes(temperature_c, eta_norm, group = sample_id, colour = polyol)) +
  ...
  scale_y_log10(breaks = c(1, 1.5, 2, 3, 4, 6, 8), labels = label_number(accuracy = 0.1)) +
  scale_x_continuous(breaks = seq(45, 75, 5), expand = expansion(mult = c(0.01, 0.02))) +
  labs(x = "Temperature (°C)", y = expression(eta[T] / eta[75])) +
  annotate("text", x = 45.7, y = 7.2, label = "within source range", hjust = 0,
...
pB <- ggplot(fits, aes(r2, polyol, colour = polyol)) +
  ...
  annotate("text", x = 0.977, y = 3.32, label = "37/39 ≥ 0.98", hjust = 1,
           size = 2.65, colour = PUR_COL[["ink"]]) +
  theme(legend.position = "none", plot.margin = margin(4, 6, 7, 5))

pC <- ggplot(fits, aes(polyol, Ea_kJmol, colour = polyol)) +
  geom_boxplot(width = 0.50, outlier.shape = NA, linewidth = 0.52,
               fill = "white", colour = PUR_COL[["ink"]]) +
  ...
  annotate("text", x = 1.02, y = 96.5, label = "Overall median = 51.09", hjust = 0,
           size = 2.55, colour = PUR_COL[["ink"]]) +
