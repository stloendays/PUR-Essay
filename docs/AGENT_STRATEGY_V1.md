...
The Agent is a **blinded scientific decision-recovery system**, not an optimizer that owns the answer. The deterministic science layer (PUR-FRONTIER V1) is now frozen first; the Agent is evaluated afterwards.
...
3. **Complete-decision recovery** — property winner, constrained winner, robust winner, active constraint, backward threshold, reachable grid value, reachability verdict, NCO direction and composition direction. A lucky top-1 guess is not a complete success.
4. **Trace gating** — the full Agent cannot finalise until required deterministic tool families have been inspected; ablations remove both the tool and its trace requirement so failures remain measurable rather than impossible to execute.
5. **Robustness recovery** — the current primary benchmark uses the frozen PUR-FRONTIER V1 robust rule. `rank_robust` must therefore return an L2 winner. Explicit robustness abstention remains only a safety behavior for a different/unfrozen config and is not the expected current result.
6. **Independent evaluation** — `complete_decision_recovery` is the primary metric; component metrics diagnose why a run succeeds or fails.
7. **Wet-lab separation** — prospective measurements remain excluded from the primary benchmark.
...
- `domain_ratio <= 1.0`;
- worst-case objective with log10-radius multipliers `(1, 1, 2)` for eta80, eta120 and eta80/eta120.
...
| `pur_agent` | all | yes | plan -> inspect -> constrain -> robust rank -> backward -> self-check -> final |
...
`--provider auto` uses the Responses API and can fall back to a compatible endpoint transport without changing the scientific task, tools or evaluator.
